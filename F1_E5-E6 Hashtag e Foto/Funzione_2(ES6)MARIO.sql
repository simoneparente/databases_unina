...
DECLARE
    ...
    numURI       INTEGER;
    cursURI CURSOR FOR SELECT DISTINCT uri
                       FROM f.foto;
...
BEGIN
    ...
    FOR i in 1..numURI
        LOOP
            ...
            match = 0;
            FOR j in 1..numParoleSTR
                LOOP
                    parolaSTR = split_part(stringa, '@', j);
                    RAISE NOTICE 'Parola Attuale Stringa: {%}', parolaSTR;
                    IF EXISTS(SELECT *
                              FROM f.tagfoto as T
                                       NATURAL JOIN f.foto as F
                              WHERE F.uri = currentURI
                                AND T.parola = parolaSTR) THEN
                        match = match + 1;
...
select regexp_count('prova11@prova12@provatabelle1e3', '@');