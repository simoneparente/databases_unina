...
--Scrivere una funzione che prenda in input un albero e un nodo e restituisca la somma dei label dei nodi del cammino dalla radice al nodo.
...
BEGIN
    ...
    FROM e.Albero AS A
             NATURAL JOIN e.Nodo AS N
    WHERE albero = A.coda;
    somma_nodi = somma_nodi + labelv; --sommo il label della radice

    WHILE cursore != root_a
        LOOP
            --salgo di nodo in nodo fino a raggiungere la radice precedentemente trovata
            SELECT A.padre INTO padre
            FROM e.Nodo AS N
                     NATURAL JOIN e.Arco AS A
            WHERE figlio = cursore;
            SELECT N.label INTO labelv
            FROM e.Nodo AS N
...
DECLARE
    ...
    n      INTEGER := 0; --numero di foglie dell'albero (da usare come max del loop)
...
BEGIN
    n := (SELECT COUNT(*) --numero di foglie dell'albero
          FROM (SELECT N.CodN
                FROM e.Nodo AS N
                WHERE coda = cod_albero
                EXCEPT
                SELECT A.padre
                FROM e.Arco AS A) AS Q);
...
--Dato in input un albero, restituisce la somma massima dei label dei nodi di un cammino dalla radice ad una foglia.
SELECT e.max_cammino(3);